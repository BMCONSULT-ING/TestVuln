...
from config_gold import MT5_CONFIG, SYMBOLS, TIMEFRAME, INDICATORS, ENGULFING, DYNAMIC_SLTP, RISK_GOLD, BOT_GOLD
from strategy_btc_dynamic import get_signal, get_dynamic_sltp, detect_reversal, compute_lot
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(BOT_GOLD["log_file"], encoding="utf-8"),
...
def run():
    log.info("=" * 65)
    log.info("BOT GOLD | XAUUSD | SMA20/50 + Engulfing + SL/TP Dynamiques")
    log.info("=" * 65)

    if not connect():
        return

    risk_usd = RISK_GOLD["capital"] * (RISK_GOLD["risk_pct"] / 100)
    log.info(f"Symbole: XAUUSD | Risk/Trade: ${risk_usd:.0f} | RR min: {DYNAMIC_SLTP['min_rr']}")
    ...
    while True:
        try:
            for symbol in SYMBOLS:
                ...
                if state["phase"] == "IDLE":
                    ...
                    if signal:
                        ...
                        log.info(
                            f"  -> ENTREE {signal} | "
                            f"SL:{sltp['sl']:.2f} TP:{sltp['tp']:.2f} | "
                            f"RR:1:{sltp['rr']} | Lot:{lot} | Methode:{sltp['method']}"
                        )
                ...
                # ── IN_TRADE ─────────────────────────────────────────
                    ...
                    bot_pos   = [p for p in positions if p.magic == BOT_GOLD["magic_number"]]
        ...
        time.sleep(BOT_GOLD["check_interval"])
